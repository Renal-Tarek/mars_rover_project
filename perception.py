...
def color_thresh_rock(img):
    ...
    color_select = np.zeros_like(img[:,:,0])
    above_thresh = ((img[:,:,0]==20) &(img[:,:,1] ==172)&(img[:,:,2] ==197))\
                | ((img[:,:,0]==64) &(img[:,:,1] ==173)&(img[:,:,2] ==196)) 
...
def perception_step(Rover):
    ...
    if len(x_pix_rock)>=5:
        Rover.found=True
        Rover.Rock_dist, Rover.Rock_angle=to_polar_coords(x_pix_rock,y_pix_rock)
    if Rover.Home_pos==None:
